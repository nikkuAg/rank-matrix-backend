...
# Import all models
from .models import Category, College_Category, Gender, Updates, College_Type, getLatestYear

# Imprting all serializers
from .serializers import CategorySerializer, GenderSerializer, QuotaSerializer, UpdatesSerializer
...
class QuotaViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        institute_code = self.request.query_params.get(
            'institute_code', DEFAULT_NULL)
        print(institute_code)
        if(institute_code == DEFAULT_NULL):
            queryset = College_Category.objects.filter(
                institute_code__category=institute_type).values('quota').distinct()
        else:
            queryset = College_Category.objects.filter(
                institute_code=institute_code).values('quota').distinct()
...
def availableRound(request):
    if(request.method == "GET"):
        ...
        for round in models_list['rounds_' + str(year)]:
            ...
            round = round_name[:len(round_name)-1] + \
                " " + round_name[len(round_name)-1]
            rounds.append(round)
...
def create(request, key):
    if(request.user.is_staff and request.user.is_superuser):
        ...
        files = [name for name in os.listdir(
            "database/") if os.path.splitext(name)[1] == '.csv']
        filesRounds = [name for name in os.listdir(
            "database/Rounds") if os.path.splitext(name)[1] == '.csv']
            ...
            for model in models:
                ...
                try:
                    if files.index((model_name + '.csv')) >= 0:
                        database_name = 'database/' + model_name + '.csv'
                        data = pd.read_csv(
                            database_name, sep=',', header=0, na_filter=False)
                except:
                    try:
                        if filesRounds.index((model_name + '.csv')) >= 0:
                            database_name = 'database/Rounds/' + model_name + '.csv'
                            data = pd.read_csv(
                                database_name, sep=',', header=0, na_filter=False)
                    except:
                        data = None
