...
class branchOneOneViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        institute_id = self.request.query_params.get(
...
def one_one(request):
    if request.method == "GET":
        ...
        if(institute_id != DEFAULT_NULL and branch_id != DEFAULT_NULL):
            ...
            branch_detail = list(Branches.objects.filter(id=branch_id).values(
                ...
                for model in models:
                    try:
                        one_one_data = list(model.objects.filter(institute_code=institute_id, branch_code=branch_id,
                                            quota=quota, category=category, seat_pool=seat_pool).values('opening_rank', 'closing_rank'))[0]
                    except:
                    ...
                    if(rank == DEFAULT_NULL or one_one_data['opening_rank'] == 0):
